...
{
    NSArray *array=[self.month componentsSeparatedByString:@"-"];
    return [self.chineseMonths objectAtIndex:[[array objectAtIndex:1]intValue]-1];
}

- (NSString *) getChineseDay
{
    return [self.chineseDays objectAtIndex:[[self day]intValue]-1];
}
